...
class WorkingUser {
    ...
    public final int getPMKeyS(String input) {
        ...
        if (input == null || input.equals("") || !isLong(input) || !PersonDatabase.getInstance().personExists(Long.parseLong(input))) { // checks for valid numbers in the PMKeyS
            user = null;
            return 1;
        } else {
            user = PersonDatabase.getInstance().readDatabasePerson(Long.parseLong(input));
        }
    ...
    public final String[] getUserNames() {
        return PersonDatabase.getInstance().getUserNames();
    }
    ...
    public final Person getUser(String name) {
        return PersonDatabase.getInstance().readDatabasePerson(name);
    }
    ...
    public final String[] getProductNames() {
        return ProductDatabase.getInstance().getProductNames();
    }
    ...
    public final boolean passwordsEqual(String PW) {
        ...
        return (testing.equals(PersonDatabase.getInstance().getPersonName(-2)));
    }
    ...
    public final void setAdminPassword(String PW) {
        PersonDatabase.getInstance().setAdminPassword(PW);
    }
    ...
    public ScrollPane printDatabase(String type) throws IOException {
        ...
        switch (type) {
            case ("Product"):
                textArea = new TextArea(ProductDatabase.getInstance().getDatabase());
                break;
            case ("Person"):
                textArea = new TextArea(PersonDatabase.getInstance().getDatabase());
                break;
            default:
                textArea = new TextArea(PersonDatabase.getInstance().getDatabase());
                break;
    ...
    public final void buyProducts() {
        ...
        ProductDatabase.getInstance().writeOutDatabase(purchased);
        PersonDatabase.getInstance().writeOutDatabasePerson(user);
    ...
    public final boolean addToCart(String input) {
        ...
        Product adding = ProductDatabase.getInstance().getProductRef(tempBarCode);
        if (adding != null) {
    ...
    public final void addPersonToDatabase(String name, long PMKeyS) {
        PersonDatabase.getInstance().setDatabasePerson(name, 0, 0, PMKeyS, true);
    }
    ...
    public final void addProductToDatabase(String name, long barCode, long price) {
        ProductDatabase.getInstance().setDatabaseProduct(name, price, barCode);
    }
    ...
    public final void changeDatabaseProduct(String name, String oldName, long price, long barcode, long oldBarcode) {
        ProductDatabase.getInstance().changeDatabaseProduct(name, oldName, price, barcode, oldBarcode);
    }
    ...
    public final void changeDatabasePerson(String selectedIndex, String name, long pmkeys, long oldPmkeys) {
        PersonDatabase.getInstance().changeDatabasePerson(selectedIndex, name, pmkeys, oldPmkeys);
    }
    ...
    public final void adminWriteOutDatabase(String type) throws IOException {
        switch (type) {
            case ("Person"):
                PersonDatabase.getInstance().adminWriteOutDatabase("adminPersonDatabase.csv");
                break;
            case ("Product"):
                ProductDatabase.getInstance().adminWriteOutDatabase("adminProductDatabase.csv");
                break;
            default:
                PersonDatabase.getInstance().adminWriteOutDatabase("adminPersonDatabase.csv");
        }
    ...
    public final void removePerson(String index) throws IOException, InterruptedException {
        PersonDatabase.getInstance().delPerson(index);
    }
    ...
    public final void removeProduct(String index) throws IOException, InterruptedException {
        ProductDatabase.getInstance().delProduct(index);
    }
    ...
    public final long getProductBarCode(int index) {
        return ProductDatabase.getInstance().getBarCode(index);
    }
    ...
    public final long getProductBarCode(String name) {
        Product getting = ProductDatabase.getInstance().readDatabaseProduct(name);
    ...
    public final String getProductName(String index) {
        Product getting = ProductDatabase.getInstance().readDatabaseProduct(index);
    ...
    public final double getProductPrice(String index) {
        return ProductDatabase.getInstance().getProductPrice(index);
    }
    ...
    public final int getProductNumber(String name) {
        return ProductDatabase.getInstance().getNumber(name);
    }
    ...
    public final void setNumberOfProducts(String name, int numberOfProducts) {
        ProductDatabase.getInstance().setNumber(name, numberOfProducts);
    }
    ...
    public final boolean userCanBuyAdmin(String name) {
        Person usr = PersonDatabase.getInstance().readDatabasePerson(name);
        return usr.canBuy();
    }
    ...
    public final void setUserCanBuy(String userName, boolean canBuy) {
        PersonDatabase.getInstance().setPersonCanBuy(userName, canBuy);
    }
    ...
    public final void resetBills() {
        PersonDatabase.getInstance().resetBills();
    }
    ...
    public final void resetRunningCosts() {
        PersonDatabase.getInstance().resetRunningTotals();
    }
