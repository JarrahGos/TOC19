...
public class PersonDatabase
{
	...
	public int setDatabasePerson(int personNo, String name, double running, double week, long barCode) // take the persons data and pass it to the persons constructor
	{
		...
		*/
		...
		else return 1; // Something went wrong, probably that user trying to add two persons with the same name and artist. Now I have to tell them off.
	}
	public String getDatabase(int sort) 
	{
		/**
		Class PersonDatabase: Method getDatabase
	...
	{
		/**
		Class PersonDatabase: Method personsUnder
		Precondition: setDatabase has been run, paremeters time and sort type have been passed as double and int respectively
		Postcondition: the user will be given a list of all of the persons under the specified time in the order requested.
		*/
	...
	{
		/**
		Class PersonDatabase: Method getPerson
		Preconditions: setDatabase has been run, paremeter is an interger between from 1 to 4
		Postconditions: the user will see the details of their chosen person output.
		*/
	...
	{
		/**
		Class PersonDatabase: Method delPerson
		Preconditions: setDatabase has been run, personNo is an integer paremeter
		Postconditions: the chosen person will no longer exist. The success or failure of this will be given by a 0 or 1 returned respectively.
		*/
		...
		else return 1; // Telling the user yet again that their person doesn't exist. At least thay don't need to delete it now.

	}
	public String getPersonName(int personNo) 
	{
		/**
		Class PersonDatabase: Method getPersonName
		Preconditions: setDatabase has been run for the invoking person
		Postconditions: the person name will be returned
		*/
		if(personNo < logicalSize) { // check that the desired person exists
	...
	{
		/** 
		Class PersonDatabase: Method getPersonSize
		Preconditions: setDatabase has been run for the invoking person
		Postconditions: the size of the invoking person will be returned as a double
		*/
	...
	public long getBarCode(int personNo)
	{
		...
		Precondition: setDatabase has been run for the invoking person
		Postcondition: the duration of the invoking person will be returned as a double
		*/
		if(personNo < logicalSize) { // to understand its workings, see getPersonName. it works the same but this returns a double
	...
	{
		/**
		Class PersonDatabase: Method emptyPerson
		Preconditions: none
		Postconditions: the number of the first found empty person will be returned an as integer or -1 will be returned if there are no empty persons
		*/
	...
	public int partitionByCost(int lb, int  ub)
	...
	public int findPerson(long barCode) 
	{
		...
		for(i = 0; i < logicalSize; i++) 
			if(allPersons[i].getBarCode() == barCode) return i;
		return -1;
	}
